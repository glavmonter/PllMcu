...
void rtt_log_buffer_hex_internal(const char *tag, const void *buffer, uint16_t buff_len,
                                 rtt_log_level_t log_level)
{
    if (buff_len == 0) {
        return;
    }
    ...
    do {
        ...
        RTT_LOG_LEVEL(log_level, tag, "%s", hex_buffer);
        l_buffer += bytes_cur_line;
        buff_len -= bytes_cur_line;
    } while (buff_len);
}

void rtt_log_buffer_char_internal(const char *tag, const void *buffer, uint16_t buff_len,
                                  rtt_log_level_t log_level)
{
    if (buff_len == 0) {
        return;
    ...
    do {
        ...
        RTT_LOG_LEVEL(log_level, tag, "%s", char_buffer);
        l_buffer += bytes_cur_line;
        buff_len -= bytes_cur_line;
    } while (buff_len);
}
...
void rtt_log_buffer_hexdump_internal(const char *tag, const void *buffer, uint16_t buff_len, rtt_log_level_t log_level)
{
    ...
    do {
        ...
        RTT_LOG_LEVEL(log_level, tag, "%s", hd_buffer);
        l_buffer += bytes_cur_line;
        buff_len -= bytes_cur_line;
    } while (buff_len);
}
