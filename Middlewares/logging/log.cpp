 ...
 *
 * Log library stores all tags provided to rtt_log_level_set as a linked
...
typedef struct uncached_tag_entry_ {
    ...
    uint8_t level;  // rtt_log_level_t as uint8_t
    char tag[0];    // beginning of a zero-terminated string
} uncached_tag_entry_t;

rtt_log_level_t rtt_log_default_level = CONFIG_LOG_DEFAULT_LEVEL;
...
static inline bool get_cached_log_level(const char *tag, rtt_log_level_t *level);
static inline bool get_uncached_log_level(const char *tag, rtt_log_level_t *level);
static inline void add_to_cache(const char *tag, rtt_log_level_t level);
static void heap_bubble_down(int index);
static inline void heap_swap(int i, int j);
static inline bool should_output(rtt_log_level_t level_for_message, rtt_log_level_t level_for_tag);
...
void rtt_log_level_set(const char *tag, rtt_log_level_t level)
{
    ...
    if (it == NULL) {
        ...
        uncached_tag_entry_t *new_entry = (uncached_tag_entry_t *) (new uint8_t[entry_size]);
        if (!new_entry) {
            rtt_log_impl_unlock();
            return;
        }
...
#ifdef LOG_BUILTIN_CHECKS
        assert_param(i == 0 || s_log_cache[(i - 1) / 2].generation < s_log_cache[i].generation);
#endif
...
/* Common code for getting the log level from cache, rtt_log_impl_lock()
   should be called before calling this function. The function unlocks,
   as indicated in the name.
*/
static rtt_log_level_t s_log_level_get_and_unlock(const char *tag)
{
    rtt_log_level_t level_for_tag;
    ...
    if (!get_cached_log_level(tag, &level_for_tag)) {
        if (!get_uncached_log_level(tag, &level_for_tag)) {
            level_for_tag = rtt_log_default_level;
        }
...
void rtt_log_writev(rtt_log_level_t level,
                    const char *tag,
                    const char *format,
                    va_list args)
{
    if (!rtt_log_impl_lock_timeout()) {
        return;
    }
    rtt_log_level_t level_for_tag = s_log_level_get_and_unlock(tag);
...
void rtt_log_write(rtt_log_level_t level,
                   const char *tag,
                   const char *format, ...)
{
    va_list list;
    va_start(list, format);
    rtt_log_writev(level, tag, format, list);
    va_end(list);
}
...
static inline bool get_cached_log_level(const char *tag, rtt_log_level_t *level)
{
    ...
    *level = (rtt_log_level_t) s_log_cache[i].level;
...
static inline bool get_uncached_log_level(const char *tag, rtt_log_level_t *level)
{
    ...
    SLIST_FOREACH(it, &s_log_tags, entries) {
        if (strcmp(tag, it->tag) == 0) {
            *level = static_cast<rtt_log_level_t>(it->level);
            return true;
        }
    }
    return false;
